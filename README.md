...
- **Vroom: månadens siffror** — egen flik i karusellavdelningen med **månadens tio mest registrerade elbilar** (`GET /api/vroom-top-cars`) och månadens nyheter ur Vrooms pressrum (`GET /api/vroom-news`). Uppdateras av sig själv: båda källorna är RSS och cachen är sex timmar, så en ny månad syns utan att någon rör koden. Ingen schemalagd synk — tjänsten ligger på gratisnivån och sover, så ett cron-uttryck hade inte garanterat något. Svarar ingen källa ritas ingen flik alls
...
- **Modellraden i märkesväljaren bär verklig räckvidd** (2026-08-29) — raden sa förut bara `58 kWh · AC 11 kW · DC 150 kW`, alltså tre laddsiffror och ingenting om hur långt bilen faktiskt går. Nu står **milen först och batteriet efter** (`~56 mil verklig · 79 kWh`) med laddeffekten ett steg svagare på egen rad, och en förklaring står **en gång** ovanför listan i stället för på varje rad: räckvidden är verklig, WLTP minus 15 %, samma `verkligaMil()` som "Din elbil" och laddkalkylen använder — inte en fjärde kopia av 0,85. **Täckningen är mätt innan raden byggdes:** alla **520** bilar i `/api/cars` har både `rangeKm` och `batteryKwh`, så inget kort behöver stå tomt; saknas talet ändå skrivs "räckvidd saknas" ut i stället för att raden tyst blir kortare, eftersom en rad som bara tappar milen ser identisk ut med en bil vi inte har. **Tre bilar har `maxDcKw = 0`** (Zoe 22 kWh, Zoe ZE40, Renault 5 40 kWh 95 hk) och fick förut "DC 0 kW", vilket läser som en mätning som gått fel — de säger nu "ingen snabbladdning", en sak man vill veta innan man väljer bilen. Rullgardinen bakom väljaren fick samma tal, så de två aldrig kan säga olika saker
- **Splashen håller kvar tills datan finns** (2026-08-20) — `ev-app.js` och `ev-splash.js` **serveras av CarAdvice**, inte härifrån: Elbilsladdning ligger på Renders gratisnivå och somnar efter ~15 min, och renderingsblockerande taggar från en sovande värd gav en sida där bara rubriken syntes. Filerna finns därför i **två repon och måste vara identiska** — ett driftprov jämför kopiorna här med dem som faktiskt serveras. Tre delar: **`ASSETS` är skilt från `API`** — datan bor här, filerna hos CarAdvice, och basen härleds ur `document.currentScript` så samma fil fungerar från båda värdarna; **kallstartsvakt** — en probe mot `/api/health` med tidsgräns 900 ms sätter `EV_COLD_START` och visar splashen även för en återkommande besökare (en tom sida läser som ett fel, en splash läser som att något händer), och anropet väcker dessutom tjänsten tidigare än första datahämtningen hade gjort; **splashen släpper på datan, inte på animationen** — `evDataKlar()` sätter `EV_DATA_READY` och skickar `ev-data-ready` **både vid träff och vid miss** (ett misslyckat anrop är också ett slut på väntan), och `narDataFinns()` väntar in signalen. Bakgrund: animationen tar ~5,5 s men billistan kom först vid 8,5 s under en uppmätt kallstart — 1,7 s där appen stod tom efter att splashen släppt, alltså precis det tomrum den fanns till för. **Taket är två tal, för hur länge det är rimligt att vänta beror på varför vi väntar:** svarade tjänsten snabbt på proben och datan ändå dröjer är något sönder längre bak, och då räcker **9 s**; är `EV_COLD_START` satt vaknar tjänsten och väntan har ett känt slut, och då gäller **20 s** — medvetet kortare än uppvakningen, inte en gissning om den. Uppvakningen är uppmätt till **121,2 s** (webbläsare, till billistan fanns) och **114,99 s** (`curl` mot `/api/health`) den 2026-08-20, båda mot en bevisat nedspunnen tjänst; talen 13,3 s och 73 s samma dag är alltså utstickare. **Inget tak kan täcka två minuter utan att låsa besökaren bakom en splash i över en minut** — och ett tak på 75 s brann av före datan varje gång ändå, så den långa väntan gav ingenting. Splashen täcker därför animationen plus en kort respit och släpper sedan fram en **läsbar** sida. Därför bär gränssnittet den väntan som blir kvar — **`bilVantetext()` i `ev-app.js` sätter rullgardinen till "Tjänsten startar — bilarna dyker upp strax…"** om `/api/cars` inte svarat inom 1 200 ms, och återställer originaltexten när bilarna kommer. Utan den stod sidan färdigladdad med "Välj bilmodell…" och noll bilar i **70 sekunder** efter att splashen gett upp — samma fel som splashen byggdes mot, ett lager längre ned. Funktionen returnerar sin egen återställare i stället för att någon annan ska minnas originaltexten. **Väntan SYNS numera också** (2026-08-30): klassen `ev-picker-vantar` på märkesväljarens rot tänder en roterande ring runt blixtplattan, ett ljussvep över avtryckaren och ett skimmer genom texten. Skälet är att texten ensam inte räckte — en knapp som byter ord men i övrigt ser exakt likadan ut läser fortfarande som en sida som står still, och det är just "står still" beskedet ska motbevisa. Tre detaljer är medvetna: svepet ligger i `background-image` och inte i ett absolut `::after`, eftersom ett positionerat pseudoelement målas **ovanpå** texten och tvättar ur den; skimret står bakom en `@supports`-vakt för `background-clip:text`, annars blir `color:transparent` en osynlig text där stödet fattas; och `prefers-reduced-motion` stoppar rörelsen men behåller ringen och den tända ramen, för rörelsen är en förklaring och inte en dekoration. Klassen sitter på **roten** och inte på avtryckaren, som `valj()` skriver över, och släcks av en gemensam `slutaVanta()` i både `fyll()` och `fel()` — ett felmeddelande som fortsätter snurra säger två motsatta saker samtidigt. Under kallstart står det dessutom **"tjänsten vaknar — det kan ta en stund…"** i stället för "väntar på datan…", eftersom vi vet varför det dröjer. **Stapeln fylls till 92 % av animationen och till 100 % först av `finish()`** — de sista åtta procenten *är* datan, och förut stod den på "100 % laddat" under hela väntan, alltså läste kortet som klart medan det enda som saknades var just det man väntade på. Skip-knappen går medvetet förbi väntan så att besökaren alltid kan ta sig ur. **42 prov** körs med `node backend/src/test/js/kallstart-prov.js`; provet klipper ut de riktiga blocken ur `ev-app.js` och `ev-splash.js` och kör dem, så ingen kopia av logiken finns i provet
...
- **Parallell exekvering** — OCM och NOBIL körs samtidigt; prisberikning för alla 5 stationer körs parallellt via virtuella trådar (`newVirtualThreadPerTaskExecutor`); sparar ~1–2 s per sökning vid cold cache
...
127 backendtester i fyra lager — ren logik, repository mot H2, HTTP-felvägar och controller-lagret (MockMvc, tjänsterna mockas). **Tabellen summerar till totalen**, så en klass som glöms bort syns som ett glapp:
...
| `EvSpecRepositoryTest` (2) | Frågan mot en riktig databas (H2) och inte en mock: NULL i `car_type` räknas som elbil, och den gamla frågan tappade de otypade raderna |
| `OperatorPriceServiceTest` (15) | Operatörsmatchning, stationsnamnsfallback, OCM-placeholder ignoreras |
| `GroqServiceTest` (24) | Promptbygget (topplistor, stationslista, kostnadsjämförelse), fallbacksvar, 429-retry-parsning, sektionsextraktion. **Chattprompten får inte gissa specar ur modellnamnet** (2026-08-29): utan kontext svarade assistenten "1,2–1,3 timmar" på hur länge en **Volvo EX60** laddar från 0 % med 50 kW — räknat på ett **påhittat 60 kWh-batteri** hämtat ur namnet. Bilen har 112 kWh och tar 2 tim 14 min, vilket samma fråga MED kontext också svarade, så felet uppstår bara när ingen bil är vald. Två hål bakom det: prompten sa **"73 modeller i databasen"** medan tabellen bar **520**, och **batteristorleken fanns inte på en enda rad** i den — topplistorna bar kW, km och pris men aldrig kWh. Nu räknas antalet ur `cars.size()`, topplistorna bär `kWh batteri`, och en egen regel förbjuder att härleda batteriet ur namnet. **Bilar som nämns vid namn slås upp per fråga** (`namndaBilar`): kontexten bär bara den bil som är VALD i väljaren, så en fråga om en bil man inte valt hade inga siffror alls — och alla 520 rader får inte plats i prompten (Groqs tak är 8 000 tokens/minut). Blocket `NÄMNDA BILAR` byggs därför ur de **tre senaste ANVÄNDARraderna** (följdfrågan upprepar sällan namnet; assistentens egna svar genomsöks inte, annars hade en bil den själv råkat nämna bekräftat sig själv). Träffas på **hela namnet eller modellbeteckningen utan märke** — kravet är formulerat på vad som gör en beteckning unik, inte på ordlängd: siffra + bokstav räcker från två tecken (`i3`, `q4`, `ex30`), rena bokstäver kräver tre och att ordet inte är generiskt (`zoe`, `kona` — men aldrig `model`, som står på var enda Tesla-rad), och **en ensam siffra duger aldrig**, så `Polestar 2` kräver att märket står med. Märkesordet hoppas normalt över, men `MG4`/`MG5` bär modellen i första ordet och är inventerade som de enda två i tabellen som gör det. **Träffarna grupperas per familj och plockas varvat**, mätt mot de riktiga 520 namnen: "skillnaden mellan Škoda Enyaq och en ID.4?" ger 12 träffar och "Polestar 2 jämfört med Tesla Model 3" ger 8 — en rak lista hade fyllt taket (6) med den ena bilens varianter och kapat bort den andra ur en fråga som gäller båda. Familjen som stavades ut går först, så `Audi Q4 e-tron` inte trängs undan av `Audi e-tron GT` som fastnar på ordet e-tron. **Falsklarmen mättes till noll** på sex bilfria frågor, inklusive fällorna "ladda till 2 procent" och "vilken model är bäst". Prompten byggs numera på ETT ställe (`byggSysPrompt`) i stället för som två identiska kopior i `chat` och `chatStream` — streaming är normalvägen och den andra är reservvägen, alltså precis där en glidning märks sist |
...
| `BlocketUsedPriceClientTest` (9) | Annonsfiltret: fel årsmodell faller även när Blocket själv påstår motsatsen, slitna exemplar faller på milgränsen, månadsavgifter och felskrivna priser faller, laddhybrider faller på bränslefiltret, och trimfiltret läser `model_specification` — inte rubriken — så procenten jämförs mot rätt nypris |
...
| `ChargingControllerTest` (13) | Billistans form, bilindexvalidering 400, prisberikning i stationsflödet, chattens rate limit → 429. **Stationssökningen kan inte längre hänga eller tiga** (2026-08-30). Symptomet var noll stationer i appen; uppmätt i drift gav identiska anrop växelvis 5 och 0 stationer, och **ett anrop stod öppet i över nio minuter utan att svara**. Tre fel i samma väg: (1) **ingen HTTP-klient hade tidsgräns** — `RestClient.create()` har varken anslutnings- eller lästidsgräns, så en tyst källa håller tråden i all evighet. `HttpTimeouts` ger nu 4 s/8 s till OCM, NOBIL, Chargeprice och API Ninjas, och 5 s/30 s till Groq, som skriver text och behöver mer tid. (2) **Inget tak på futurerna** — `KALLA_TAK_S = 12` gäller även om en källa hänger någon annanstans än i läsningen; det är svarstiden mot användaren som räknas. (3) **Ett gemensamt `catch` utan loggrad** kring båda källornas `get()` — grenarna är nu skilda åt och båda loggar. **Att felet var osynligt var det dyra:** Render-loggen såg frisk ut hela vägen och tjänsten svarade 200 med en glad AI-rekommendation ovanför tomrummet. Källorna väger dessutom olika — OCM ÄR stationslistan, NOBIL bidrar bara med antalet kontakter per station — så bara den ena får tömma svaret. (I praktiken var det alltid OCM: `NobilService` sväljer sina egna undantag och returnerar tom lista, så NOBIL-grenen kunde aldrig falla ut hit. Uppdelningen är alltså lika mycket till för att man ska KUNNA SE vilken källa som fallerat.) **Och tomt måste kunna SÄGA varför** — `sourceError` i `StationResponse` sätts BARA när källan fallerade, aldrig när den svarade med noll stationer. De två ser likadana ut i en tom lista men betyder motsatta saker, och gränssnittet visade förut ingenting alls i båda fallen: sidan såg färdig ut, och enda rimliga slutsatsen för den som läste den var att det inte finns laddare i närheten. Nu står det antingen "Laddstationsdatan (OpenChargeMap) svarar inte just nu" eller "Inga laddstationer som passar din bil inom 15 km". **Bakgrund: 2026-08-30 slutade OpenChargeMap svara helt** — mätt både från Render och från en helt annan uppkoppling: `/v3/poi/` UTAN nyckel svarar 403 på 0,09 s (värden lever), medan samma fråga MED giltig nyckel hängde i över 60 s, fem varianter i rad (nyckel i query och i header, `compact=true`, minimal fråga, `countrycode=SE`). Det är alltså källans dataväg som ligger nere, inte vår tjänst och inte nyckeln. |
| `FavoriteControllerTest` (5) | Dubblettskydd 409, ägarkontroll vid borttagning 404, spara/lista/ta bort |
| `VroomNewsServiceTest` (9) | Vrooms pressrum via Mynewsdesks RSS. **"Senaste månaden" räknas ur flödet, inte ur kalendern** — Vroom publicerar 2–4 gånger i månaden, så den 1:a i en ny månad hade en kalenderfråga gett en tom karusell i en vecka varje månad. Räcker nyaste månaden inte till tre slides fylls listan på bakåt, och etiketten blir då ett spann ("augusti–september 2026") eftersom "september" över en augustirad är fel om än bara med en rad. **Veckodagen får inte fälla en post** (fyndet som provet gav): `DateTimeFormatter.RFC_1123_DATE_TIME` validerar veckodagen och KASTAR när den inte stämmer med datumet, så en felskriven veckodag hade tagit bort nyheten helt och tyst — och en karusell som saknar den nyaste raden ser ut precis som en karusell i fas. Veckodagen strippas nu och resten tolkas om. Dessutom: utm-parametrarna följer inte med till länken, poster utan `pubDate` hoppas över (utan datum finns ingen månad att placera raden i), och ett tomt svar behåller källfälten så att "inget nytt" inte läser som "trasigt" |
| `VroomTopCarsServiceTest` (9) | Månadens tio mest registrerade elbilar. **Siffrorna är Vrooms, men de går inte att hämta hos Vroom:** pressreleasen ger marknaden i aggregat och modell-topplistan ligger i deras betalda verktyg (kontrollerat mot både pressrummet och web.vroom.nu 2026-09-17 — ordet "topplista" står där sex gånger, varje gång som en produktbeskrivning). Studio Esse publicerar listan i klartext varje månad med Vroom som uttalad källa, och YouTubes kanalflöde bär hela beskrivningen i `media:description`. **Listan är elbilar trots att källan skriver "personbilar"** — samtliga 25 rader är BEV och en etta på 731 bilar kan inte vara en marknad på ~20 000 i månaden. **`Element.text()` plattar ut radbrytningar**, vilket gjorde att ett radankrat mönster matchade exakt noll rader; texten läses med `wholeText()` och mönstret klarar dessutom en utplattad lista. Beskrivningen skrivs för hand varje månad, så både em-dash, en-dash och bindestreck godtas, liksom hårda och smala mellanslag i tusentalet. Modellnamn med siffror och tecken (`POLESTAR 4`, `C-HR+`, `RENAULT 5`) överlever, poster utan rubrikrad hoppas över (kanalen blandar in tester och nyheter) och en halv lista räknas inte — nio rader är ingen topplista. **En vakt bevakar att listan inte byter innebörd:** beviset för att den är elbilar är att Sveriges faktiska storsäljare SAKNAS — XC60, Golf och Yaris Cross finns inte på någon av de 25 platserna, och en lista över alla personbilar utan XC60 är inte en lista över alla personbilar. Dyker någon av dem upp loggas en varning, för då är appens rubrik fel. Den **fäller ingenting** — siffrorna är fortfarande riktiga, och att tiga om dem vore värre än en felaktig rubrik |
| `VroomControllerTest` (3) | Att Vroom-endpointerna FINNS. Provet skrevs efter att `/api/vroom-top-cars` svarade **404 i drift** trots 17 gröna prov på tjänsterna under: tjänsten var byggd och testad, men ingen controller hade skrivits, och ingenting i sviten gick via HTTP. Samma läxa som drift-provet för `ev-app.js` bär — ett prov som mäter delen bevisar inte att delen är ANSLUTEN. Båda vägarna prövas nu, och dessutom att ett tomt svar är **200 med tom lista och källan kvar**, aldrig 404: frontenden skiljer på "inget att visa" (ingen flik) och "trasigt", och ett felsvar hade fått kortet att se sönder ut |
