...
126 backendtester i fyra lager — ren logik, repository mot H2, HTTP-felvägar och controller-lagret (MockMvc, tjänsterna mockas). **Tabellen summerar till totalen**, så en klass som glöms bort syns som ett glapp:
...
| `VroomTopCarsServiceTest` (8) | Månadens tio mest registrerade elbilar. **Siffrorna är Vrooms, men de går inte att hämta hos Vroom:** pressreleasen ger marknaden i aggregat och modell-topplistan ligger i deras betalda verktyg (kontrollerat mot både pressrummet och web.vroom.nu 2026-09-17 — ordet "topplista" står där sex gånger, varje gång som en produktbeskrivning). Studio Esse publicerar listan i klartext varje månad med Vroom som uttalad källa, och YouTubes kanalflöde bär hela beskrivningen i `media:description`. **Listan är elbilar trots att källan skriver "personbilar"** — samtliga 25 rader är BEV och en etta på 731 bilar kan inte vara en marknad på ~20 000 i månaden. **`Element.text()` plattar ut radbrytningar**, vilket gjorde att ett radankrat mönster matchade exakt noll rader; texten läses med `wholeText()` och mönstret klarar dessutom en utplattad lista. Beskrivningen skrivs för hand varje månad, så både em-dash, en-dash och bindestreck godtas, liksom hårda och smala mellanslag i tusentalet. Modellnamn med siffror och tecken (`POLESTAR 4`, `C-HR+`, `RENAULT 5`) överlever, poster utan rubrikrad hoppas över (kanalen blandar in tester och nyheter) och en halv lista räknas inte — nio rader är ingen topplista |
